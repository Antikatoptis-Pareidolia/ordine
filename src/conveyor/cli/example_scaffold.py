"""Scaffold the conveyor example demo directory.

Owns filesystem layout for ``conveyor example``. Must never import web or executors.
"""
...
from conveyor.llm.steps import render_mock_image
...
def scaffold_example(target: Path) -> list[str]:
    ...
    (chain / "README.md").write_text(
        ...
        f"1. `conveyor run {chain / 'gen-images.yml'} --oneshot`\n"
        f"2. `conveyor run {chain / 'png-cleanup.yml'} --oneshot`\n",
        encoding="utf-8",
    )

    return [
        "conveyor check png-cleanup.yml",
        f"conveyor run {cleanup} --oneshot",
        "conveyor serve",
    ]
