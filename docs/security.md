...
Treat playbook YAML like shell scripts: review triggers, destinations, and branch steps before `conveyor serve` on a shared host.
...
See `src/conveyor/web/security.py` and [web.md](web.md).
...
| JSONL audit log | No (local) | `~/.local/share/conveyor/llm/` |
...
2. OS keyring (`keyring` package) via Settings UI or `conveyor` key helpers
3. Never commit keys; `.env` is user-local only

## Telemetry

**None.** Conveyor does not phone home, crash-report, or analytics-track. See README privacy statement.
